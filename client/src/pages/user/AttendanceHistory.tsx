...
export default function AttendanceHistory() {
  ...
  const getStatusColor = (status: string, isHoliday: boolean = false) => {
    ...
    const colors: Record<string, string> = {
      ...
      "half-day": "bg-purple-100 dark:bg-purple-900/20 text-purple-800 dark:text-purple-200 border-purple-300 dark:border-purple-700",
    };
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <Card>
            <CardContent className="p-4">
              ...
              <div className="text-2xl font-semibold" data-testid="text-total-days">{monthlySummary.totalDays}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-sm text-muted-foreground">Present</div>
              <div className="text-2xl font-semibold text-green-600" data-testid="text-present">{monthlySummary.present}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-sm text-muted-foreground">Absent</div>
              <div className="text-2xl font-semibold text-red-600" data-testid="text-absent">{monthlySummary.absent}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-sm text-muted-foreground">Late</div>
              <div className="text-2xl font-semibold text-yellow-600" data-testid="text-late">{monthlySummary.late}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-sm text-muted-foreground">Leave</div>
              <div className="text-2xl font-semibold text-blue-600" data-testid="text-leave">{monthlySummary.leave}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-sm text-muted-foreground">Half Day</div>
              <div className="text-2xl font-semibold text-purple-600" data-testid="text-halfday">{monthlySummary.halfDay}</div>
            </CardContent>
          </Card>
        </div>
      )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left p-3 text-sm font-semibold">Check Out</th>
                ...
                <tbody>
                  {filteredRecords.map((record) => (
                    <tr key={record.id} className="border-b hover-elevate" data-testid={`record-${record.id}`}>
                      <td className="p-3 text-sm">{format(parseISO(record.date), "MMM dd, yyyy")}</td>
                      ...
                      <td className="p-3 text-sm text-muted-foreground">{record.remarks || "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
