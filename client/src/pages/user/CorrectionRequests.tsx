  ...
  const createCorrectionMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      const checkInDateTime = data.requestedCheckIn
        ? new Date(`${data.date}T${data.requestedCheckIn}:00`).toISOString()
        : null;
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="checkIn">Requested Check-In Time</Label>
                <Input
                  id="checkIn"
                  type="time"
              ...
              <div className="space-y-2">
                <Label htmlFor="checkOut">Requested Check-Out Time</Label>
                <Input
                  id="checkOut"
                  type="time"
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-semibold" data-testid="text-pending-count">{pendingCount}</div>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-semibold" data-testid="text-approved-count">{approvedCount}</div>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-semibold" data-testid="text-rejected-count">{rejectedCount}</div>
              </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Submit a request if you need to correct your attendance
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {corrections.map((correction) => (
                <Card key={correction.id} className="overflow-hidden" data-testid={`correction-${correction.id}`}>
                      ...
                      <div className="flex-1 min-w-0 space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {correction.requestedCheckIn && (
                            <div className="text-sm">
                              ...
                              <span className="font-medium">{formatTime(correction.requestedCheckIn)}</span>
                            </div>
                          )}
                          {correction.requestedCheckOut && (
                            <div className="text-sm">
                              <span className="text-muted-foreground">Requested Check-Out: </span>
                              <span className="font-medium">{formatTime(correction.requestedCheckOut)}</span>
                            </div>
                          )}
                        </div>
                        ...
                        {correction.reviewComments && (
                          <div className="text-sm bg-muted p-3 rounded-md">
                            ...
                            <p className="mt-1">{correction.reviewComments}</p>
                          </div>
                        )}

                        <div className="text-xs text-muted-foreground">
                          Submitted on {format(new Date(correction.createdAt), "MMM dd, yyyy 'at' hh:mm a")}
                        </div>
                      </div>
                    </div>
                  </CardContent>
