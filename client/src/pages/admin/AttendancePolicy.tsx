...
export default function AttendancePolicy() {
  ...
  const updatePolicyMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Update Failed",
        description: error.message || "Unable to update policy. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Attendance Policy Management</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, fullDayHours: parseInt(e.target.value) })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="lateMarkThreshold">Late Mark Threshold (minutes)</Label>
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, lateMarkThreshold: parseInt(e.target.value) })}
                  data-testid="input-late-threshold"
                />
                <p className="text-xs text-muted-foreground">
                  Grace period before marking as late (in minutes)
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="autoAbsentHours">Auto Absent After (hours)</Label>
                <Input
                  id="autoAbsentHours"
                  type="number"
                  min="1"
                  ...
                  onChange={(e) => setFormData({ ...formData, autoAbsentHours: parseInt(e.target.value) })}
                  data-testid="input-auto-absent-hours"
                />
                <p className="text-xs text-muted-foreground">
                  Mark as absent if work hours are less than this threshold
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Configure check-in/out behavior and security settings
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Enable employees to mark their own attendance
                </p>
              </div>
              <Switch
                id="allowSelfCheckIn"
                checked={formData.allowSelfCheckIn}
                onCheckedChange={(checked) => setFormData({ ...formData, allowSelfCheckIn: checked })}
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) => setFormData({ ...formData, requireGPS: checked })}
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="requireDeviceBinding">Require Device Binding</Label>
                </div>
                <p className="text-sm text-muted-foreground">
                  Restrict attendance to registered devices only
                </p>
              </div>
              <Switch
                id="requireDeviceBinding"
                checked={formData.requireDeviceBinding}
                onCheckedChange={(checked) => setFormData({ ...formData, requireDeviceBinding: checked })}
        ...
        <CardContent className="space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-medium">
              {policy?.updatedAt ? new Date(policy.updatedAt).toLocaleString() : "Never"}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Policy ID:</span>
            <span className="font-medium">#{policy?.id || "N/A"}</span>
          </div>
        </CardContent>
      </Card>
    </div>
