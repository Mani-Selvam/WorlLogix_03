...
export default function CorrectionApproval() {
  ...
  const [reviewDialog, setReviewDialog] = useState<{ open: boolean; request: CorrectionWithUser | null; action: 'approve' | 'reject' | null }>({
    open: false,
    request: null,
    action: null,
  });
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The correction request has been approved and applied.",
      });
    },
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to approve correction request. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to reject correction request. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Attendance Correction Requests</h1>
        <p className="text-muted-foreground mt-1">Review and approve employee attendance correction requests</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {corrections.map((correction) => (
          <Card key={correction.id} className="overflow-hidden" data-testid={`card-correction-${correction.id}`}>
            <CardHeader>
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-3">
                  <FileEdit className="w-5 h-5 text-muted-foreground" />
                  <CardTitle className="text-lg" data-testid={`text-employee-${correction.id}`}>
                    {correction.userName}
                  </CardTitle>
                  <Badge variant="secondary" data-testid={`status-correction-${correction.id}`}>
                    Pending Review
                  </Badge>
                </div>
                <div className="flex gap-2">
                  <Button
                    onClick={() => {
                      setReviewDialog({ open: true, request: correction, action: 'approve' });
                      setComments("");
                    }}
                  ...
                  <Button
                    onClick={() => {
                      setReviewDialog({ open: true, request: correction, action: 'reject' });
                      setComments("");
                    }}
                    disabled={approveMutation.isPending || rejectMutation.isPending}
                    variant="destructive"
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
                <div className="flex items-center gap-1">
                  ...
                  <span data-testid={`text-date-${correction.id}`}>
                    {format(parseISO(correction.date), "MMM dd, yyyy")}
                  </span>
                </div>
                <div className="flex items-center gap-1">
                  <Clock className="h-4 w-4" />
                  <span data-testid={`text-submitted-${correction.id}`}>
                    Submitted: {format(new Date(correction.createdAt), "MMM dd, yyyy 'at' hh:mm a")}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                {correction.requestedCheckIn && (
                  <div className="space-y-1">
                    <p className="text-sm font-medium">Requested Check-In</p>
                    <p className="text-sm text-muted-foreground" data-testid={`text-check-in-${correction.id}`}>
                ...
                {correction.requestedCheckOut && (
                  <div className="space-y-1">
                    <p className="text-sm font-medium">Requested Check-Out</p>
                    <p className="text-sm text-muted-foreground" data-testid={`text-check-out-${correction.id}`}>
              ...
              <div className="pt-2">
                <p className="text-sm font-medium mb-1">Reason for Correction</p>
                <p className="text-sm text-muted-foreground" data-testid={`text-reason-${correction.id}`}>
                  {correction.reason}
                </p>
              </div>
            </CardContent>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {reviewDialog.action === 'approve' ? 'Approve' : 'Reject'} Correction Request
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
