    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage company holidays and days off</p>
        </div>
      ...
      <div className="grid gap-4">
        ...
        {!isLoading && holidays.length === 0 && (
          <Card className="p-6">
            <p className="text-center text-gray-500 dark:text-gray-400" data-testid="text-no-holidays">
              No holidays added yet. Click "Add Holiday" to create your first holiday.
            </p>
          </Card>
        )}

        {holidays.map((holiday) => (
          <Card key={holiday.id} className="p-6" data-testid={`card-holiday-${holiday.id}`}>
            <div className="flex justify-between items-start">
              <div className="space-y-2 flex-1">
                <h3 className="font-semibold text-lg" data-testid={`text-holiday-name-${holiday.id}`}>
                  {holiday.name}
                </h3>
                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                  <Calendar className="h-4 w-4" />
                  <span data-testid={`text-holiday-date-${holiday.id}`}>
                ...
                {holiday.description && (
                  <p className="text-sm text-gray-700 dark:text-gray-300" data-testid={`text-holiday-description-${holiday.id}`}>
                    {holiday.description}
                  </p>
                )}
              </div>
