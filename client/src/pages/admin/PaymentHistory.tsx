...
Amount: ${payment.currency === 'INR' ? '₹' : '$'}${payment.amount.toLocaleString()}
...
`;
  ...
  return (
    <div className="space-y-6" data-testid="page-payment-history">
      <div>
        <h1 className="text-3xl font-bold mb-2" data-testid="heading-payment-history">Payment History</h1>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card data-testid="card-successful-payments">
          ...
          <CardContent>
            <div className="text-2xl font-bold" data-testid="text-successful-count">
              {successfulPayments}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <Card data-testid="card-payment-table">
        <CardHeader>
          <CardTitle data-testid="heading-transaction-history">Transaction History</CardTitle>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead data-testid="header-receipt">Receipt Number</TableHead>
                ...
                <TableBody>
                  {payments.map((payment) => (
                    <TableRow key={payment.id} data-testid={`row-payment-${payment.id}`}>
                      ...
                      <TableCell data-testid={`cell-receipt-${payment.id}`}>
                        <div className="font-mono text-sm">
                          {payment.receiptNumber || '—'}
                        </div>
                      </TableCell>
                      <TableCell data-testid={`cell-description-${payment.id}`}>
                        <div className="font-medium">
                          {payment.slotType === 'admin' ? 'Admin Slots' : 'Member Slots'}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          Quantity: {payment.slotQuantity}
                        </div>
                      ...
                      <TableCell data-testid={`cell-email-${payment.id}`}>
                        {payment.emailSent ? (
                          <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200" data-testid={`badge-email-sent-${payment.id}`}>
                            <Mail className="w-3 h-3 mr-1" />
                            Sent
                          </Badge>
                        ) : (
                          <Badge variant="outline" className="bg-gray-50 text-gray-600" data-testid={`badge-email-not-sent-${payment.id}`}>
                            Not Sent
                          </Badge>
                        )}
                      </TableCell>
                      <TableCell data-testid={`cell-actions-${payment.id}`}>
                        {payment.paymentStatus === 'paid' && payment.receiptNumber && (
                          <Button
                            variant="ghost"
                            size="sm"
