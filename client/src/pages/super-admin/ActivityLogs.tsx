...
export default function ActivityLogs() {
  ...
  const getActionIcon = (actionType: string) => {
    if (actionType.includes('company')) return <Building2 className="h-4 w-4" />;
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="activity-logs-page">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold" data-testid="page-title">Activity Logs</h1>
            ...
            <div className="space-y-4">
                ...
                >
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      <Badge variant={getActionColor(log.actionType)} data-testid={`badge-action-${log.id}`}>
                        {formatActionType(log.actionType)}
                      </Badge>
                      <span className="text-xs text-muted-foreground" data-testid={`text-date-${log.id}`}>
                        {format(new Date(log.createdAt), "MMM dd, yyyy 'at' HH:mm:ss")}
                      </span>
                    </div>
                    <p className="text-sm" data-testid={`text-details-${log.id}`}>{log.details || "No details provided"}</p>
                    <div className="flex gap-4 text-xs text-muted-foreground">
                      {log.targetCompanyId && (
                        <span>Company ID: {log.targetCompanyId}</span>
                      )}
                      {log.targetUserId && (
                        <span>User ID: {log.targetUserId}</span>
                      )}
                      <span>Performed by User ID: {log.performedBy}</span>
                    </div>
                  </div>
                </div>
              ))}
              ...
              {logs.length === 0 && (
                <div className="text-center py-12">
                  <p className="text-muted-foreground" data-testid="text-no-logs">No activity logs found</p>
                </div>
              )}
            </div>
          </ScrollArea>
