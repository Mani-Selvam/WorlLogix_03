...
export default function TeamCorrectionRequests() {
  ...
  const approveCorrection = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Error", description: error.message || "Failed to approve correction", variant: "destructive" });
    },
  });
  ...
  const rejectCorrection = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Error", description: error.message || "Failed to reject correction", variant: "destructive" });
    },
  });
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <CardDescription className="mt-2">
                      <div className="space-y-1">
                        ...
                        {(request.originalClockIn || request.correctedClockIn) && (
                          <div className="flex items-center gap-2 text-sm">
                            <span className="font-medium">Clock In:</span>
                            <span className="text-destructive line-through">{formatTime(request.originalClockIn)}</span>
                            <span>→</span>
                            <span className="text-green-600 font-medium">{formatTime(request.correctedClockIn)}</span>
                          </div>
                        )}
                        {(request.originalClockOut || request.correctedClockOut) && (
                          <div className="flex items-center gap-2 text-sm">
                            <span className="font-medium">Clock Out:</span>
                            <span className="text-destructive line-through">{formatTime(request.originalClockOut)}</span>
                            <span>→</span>
                            <span className="text-green-600 font-medium">{formatTime(request.correctedClockOut)}</span>
                          </div>
                        )}
                      </div>
                    </CardDescription>
                ...
                {request.status === "pending" && (
                  <div className="flex gap-2">
                    <Button 
                      variant="default" 
                      size="sm" 
                      onClick={() => approveCorrection.mutate(request.id)}
                      disabled={approveCorrection.isPending || rejectCorrection.isPending}
                    ...
                    <Button 
                      variant="destructive" 
                      size="sm" 
                      onClick={() => rejectCorrection.mutate(request.id)}
                      disabled={approveCorrection.isPending || rejectCorrection.isPending}
      ...
      {filteredCorrections.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            <p className="text-muted-foreground">No {statusFilter !== "all" && statusFilter} correction requests found</p>
          </CardContent>
        </Card>
      )}
    </div>
