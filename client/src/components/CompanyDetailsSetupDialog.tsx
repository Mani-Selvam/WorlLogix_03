...
export default function CompanyDetailsSetupDialog({ 
  ...
  const handleStep2Submit = async (data: Step2Data) => {
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to save address information",
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
      ...
      <DialogContent className="sm:max-w-[550px]" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {currentStep === 1 && <><Building2 className="h-5 w-5 text-indigo-600" /> Basic Company Information</>}
            {currentStep === 2 && <><MapPin className="h-5 w-5 text-indigo-600" /> Address & Location</>}
            {currentStep === 3 && <><Briefcase className="h-5 w-5 text-indigo-600" /> Business Details</>}
          </DialogTitle>
          <DialogDescription>
            {currentStep === 1 && "Step 1 of 3: Enter your company basic information"}
            {currentStep === 2 && "Step 2 of 3: Provide your business address"}
            {currentStep === 3 && "Step 3 of 3: Complete your business profile"}
          </DialogDescription>
        </DialogHeader>
          ...
          {[1, 2, 3].map((step) => (
            <div key={step} className="flex items-center">
              <div className={`h-2 w-2 rounded-full transition-colors ${
                step === currentStep ? 'bg-indigo-600 h-3 w-3' :
                step < currentStep ? 'bg-green-500' : 'bg-gray-300'
              }`} />
              {step < 3 && <div className={`h-0.5 w-12 ${
                step < currentStep ? 'bg-green-500' : 'bg-gray-300'
              }`} />}
            </div>
          ))}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="Less than 1 Lakh">Less than 1 Lakh</SelectItem>
                        ...
                        <SelectItem value="More than 5 Crores">More than 5 Crores</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
              ...
              <div className="bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500 p-4 rounded">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    <p className="font-semibold text-green-800 dark:text-green-500">Almost Done!</p>
                    <p className="text-green-700 dark:text-green-400">
                      After completing this step, you'll have full access to your dashboard.
                    </p>
                  </div>
                </div>
              </div>
